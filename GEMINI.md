...
Every `fullrun` / `fullrun new` invocation uses `pnpm josh followup --merge`, which handles the full sequence internally: wait for CI → run pre-merge review (`claude -p`) → verify AI review findings → send completion notification → merge. Invoking `fullrun` is itself the explicit authorization to merge.

- **Pre-merge review is run automatically.** After CI is green, `pnpm josh followup --merge` invokes `claude -p` with `prompts/review.md` and the PR diff. Findings tagged `(high)` or `(medium)` block the merge — a `confirmation` Telegram is sent and the command exits non-zero. Fix the findings, commit, and re-run. Pass `--review-ignore-reason "<reason>"` to bypass a design-call finding. If `claude` is not on PATH, the review is skipped with a warning.
