...
Every `fullrun` / `fullrun new` invocation uses `pnpm josh followup --merge`, which handles the full sequence internally: wait for CI → run pre-merge review (`claude -p`) → verify AI review findings → send completion notification → merge. The user does **not** need to add a keyword. Invoking `fullrun` is itself the explicit authorization to merge.

```bash
pnpm josh followup "<title> #<N>" --merge --notify-message "..."
```

- **Pre-merge review is run automatically.** After CI is green, `pnpm josh followup --merge` invokes `claude -p` with `prompts/review.md` and the PR diff. Findings tagged `(high)` or `(medium)` block the merge — a `confirmation` Telegram is sent and the command exits non-zero. Fix the findings, commit, and re-run `pnpm josh followup --merge`. Pass `--review-ignore-reason "<reason>"` to bypass a design-call finding (the reason is posted to the PR as an audit comment). If `claude` is not on PATH, the review is skipped with a warning.
- **AI review findings are checked automatically.** `pnpm josh followup --merge` scans for CodeRabbit / Claude Review / SonarQube findings before merging. If blockers are found, it sends a `confirmation` Telegram and exits non-zero — fix the findings and re-run `pnpm josh followup --merge`. **Green CI is not authorization to merge while pre-merge review or AI review findings are open.**
